...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
    ...
    */
	node_t *trav = freelist->head;
	int temp;
	block_t *fragment = malloc(sizeof(block_t*));
	while(trav->next != NULL) {
		if((trav->blk->end - trav->blk->start) >= blocksize) {
			list_remove_at_index(freelist, pid);
			trav->blk->pid = pid;
			temp = trav->blk->end;
			trav->blk->end = trav->blk->start + blocksize -1;
			
			if(policy == 1){
				list_add_ascending_by_address(alloclist, trav->blk);
			}else if(policy == 2) {
				list_add_ascending_by_address(alloclist, trav->blk);
			}else if(policy == 3) {
				list_add_ascending_by_address(alloclist, trav->blk);
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
    ...
    */
	node_t *trav = alloclist->head;
	block_t *newblk = NULL;
	while(trav->next != NULL) {
		if(trav->blk->pid == pid) {
			newblk = list_remove_at_index(alloclist, pid);
			newblk->pid = 0;
			if(policy == 1){
