...
void list_add_ascending_by_address(list_t *l, block_t *newblk){
   ...
   */
	...
	while(curNode->next != NULL) {
		if(newblk->start > curNode->blk->start){
			newNode->blk = newblk;
...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk){
   ...
   *    USE the compareSize()
   */
	int bSize = newblk->end - newblk->start + 1;
	node_t *curNode = l->head;
	node_t *newNode = NULL;
	
	int curBlocksize = curNode->blk->end - curNode->blk->start + 1;
	while(curNode->next != NULL) {
		if(bSize >= curBlocksize){
			newNode->blk = newblk;
			if(curNode->next != NULL) {
				newNode->next = curNode->next;
			}
		...
		curBlocksize = curNode->blk->end - curNode->blk->start + 1;
		curNode = curNode->next;
	}
}
...
void list_coalese_nodes(list_t *l){ 
   ...
   */
	...
	while(trav->next != NULL) {
		...
		if(prev->blk->end+1 == trav->blk->start) {
			prev->blk->end = trav->blk->end;
...
bool list_is_in_by_pid(list_t *l, int pid){ 
   ...
   */
	...
	while(trav->next != NULL) {
		...
		if(comparePid(pid, trav->blk) == true) {
			return true;
		}
		trav = trav->next;
	}
